    ...
    def assemble(self, msg):
        ...
        try:
            ...
            if platform == "linux" or platform == "linux2" or platform == "darwin":
                SCRIPT = os.path.join(FILE_DIR,'../assembler',"nix_assembler.sh")
                output = subprocess.run([SCRIPT, msg['filepath']], stderr=subprocess.PIPE)
                f.close()
                return {'status': True, 'errors': output.stderr.decode('utf-8')}
            elif platform == "win32":
                SCRIPT = os.path.join(FILE_DIR,r'..\assembler',"windows_assembler.bat")
